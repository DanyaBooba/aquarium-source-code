...
<form action="{{ route('settings.profile.email.store') }}" onsubmit="sendForm('{{ route('settings') }}')" method="post">
    ...
    <div>
        ...
        <p>
            {{ __('После отправки формы, вам потребуется подтвердить почту через ссылку в письме, после чего получить доступ к аккаунту можно будет ТОЛЬКО с новой почтой.') }}
        </p>
    </div>
